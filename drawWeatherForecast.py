...
def main():
    try:
        ...
        # Setup fonts
        # ==========================
        tahomaBig = ImageFont.truetype(
            os.path.join(fontDir, 'tahoma.ttf'), 155)
        ...
        tohoma24bd = ImageFont.truetype(
            os.path.join(fontDir, 'tahomabd.ttf'), 24)
        tohoma40 = ImageFont.truetype(os.path.join(fontDir, 'tahoma.ttf'), 40)
        weatherIconsBig = ImageFont.truetype(
            os.path.join(fontDir, 'Meteocons.ttf'), 230)
        weatherIconsSmall = ImageFont.truetype(
            os.path.join(fontDir, 'Meteocons.ttf'), 75)
        ...
        if forecastData["cod"] != "404":
            ...
            for i in forecastWeatherDataMain:
                ...
                forecastItemList.append(i["main"]["temp"])
                # Weather icon at timestamp
                forecastItemList.append(i["weather"][0]["icon"])
        ...
        # Write to E-Ink
        # ==========================
        imageBlack = Image.new(
            '1', (epd7in5bc.EPD_WIDTH, epd7in5bc.EPD_HEIGHT), 255)  # 255: clear frame
        # 255: clear frame
        imageRed = Image.new(
            '1', (epd7in5bc.EPD_WIDTH, epd7in5bc.EPD_HEIGHT), 255)
        draw = ImageDraw.Draw(imageBlack)
        # Last update stamp
        draw.text((330, 350), 'Last update: ' + currentDate +
                  ' - ' + currentTime, font=tohoma20, fill=0)
        # Black frame
        # Forecast +3h
        draw.text((20, 245), str(
            round(forecastList[0][1]-273.15, 1)) + '°C', font=tohoma20, fill=0)
        draw.text((14, 270), getWeatherIcon(
            forecastList[0][2]), font=weatherIconsSmall, fill=0)
        # Forecast +6h
        draw.text((155, 245), str(
            round(forecastList[1][1]-273.15, 1)) + '°C', font=tohoma20, fill=0)
        draw.text((149, 270), getWeatherIcon(
            forecastList[1][2]), font=weatherIconsSmall, fill=0)
        # Forecast +9h
        draw.text((290, 245), str(
            round(forecastList[2][1]-273.15, 1)) + '°C', font=tohoma20, fill=0)
        draw.text((284, 270), getWeatherIcon(
            forecastList[2][2]), font=weatherIconsSmall, fill=0)
        # Forecast +12h
        draw.text((425, 245), str(
            round(forecastList[3][1]-273.15, 1)) + '°C', font=tohoma20, fill=0)
        draw.text((419, 270), getWeatherIcon(
            forecastList[3][2]), font=weatherIconsSmall, fill=0)
        # Forecast +15h
        draw.text((560, 245), str(
            round(forecastList[4][1]-273.15, 1)) + '°C', font=tohoma20, fill=0)
        draw.text((554, 270), getWeatherIcon(
            forecastList[4][2]), font=weatherIconsSmall, fill=0)
        # CurrentWeather
        draw.text((7, 7), getWeatherIcon(weatherIcon),
                  font=weatherIconsBig, fill=0)
        draw.text((295, 170), 'Feels like ' + str(round(currentTempFeel-273.15)) +
                  '°C' + ' Humidity ' + str(currentHumidity) + '%', font=tohoma20, fill=0)
        currentTempString = str(round(currentTemperature-273.15, 1))
        currentTempCharLen = len(currentTempString)-1
        dotLength = 55
        bigFontWidth = 80
        draw.text((280+(currentTempCharLen*bigFontWidth) +
                  dotLength, 30), '°C', font=tohoma40, fill=0)
        ...
        # Forecast +3h
        draw.text((20, 220), forecastList[0]
                  [0][11:16], font=tohoma24bd, fill=0)
        # Forecast +6h
        draw.text((155, 220), forecastList[1]
                  [0][11:16], font=tohoma24bd, fill=0)
        # Forecast +9h
        draw.text((290, 220), forecastList[2]
                  [0][11:16], font=tohoma24bd, fill=0)
        # Forecast +12h
        draw.text((425, 220), forecastList[3]
                  [0][11:16], font=tohoma24bd, fill=0)
        # Forecast +15h
        draw.text((550, 220), forecastList[4]
                  [0][11:16], font=tohoma24bd, fill=0)
